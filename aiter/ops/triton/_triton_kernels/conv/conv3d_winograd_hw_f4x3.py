...
def _get_config_input(shape_key=None, M=None):
    if CONV_AUTOTUNE_ENABLED:
        return {}
    return get_conv_config("CONV3D-WINO-HW-F4X3-INPUT", shape_key=shape_key, M=M)


def _get_config_gemm(shape_key=None, M=None):
    if CONV_AUTOTUNE_ENABLED:
        return {}
    return get_conv_config("CONV3D-WINO-HW-F4X3-GEMM", shape_key=shape_key, M=M)


def _get_config_output(shape_key=None, M=None):
    if CONV_AUTOTUNE_ENABLED:
        return {}
    return get_conv_config("CONV3D-WINO-HW-F4X3-OUTPUT", shape_key=shape_key, M=M)
...
AUTOTUNE_WINO_HW_OUTPUT_CONFIGS = [
...
if CONV_AUTOTUNE_ENABLED:
    _winograd_hw_f4x3_input_transform_kernel = triton.autotune(
        configs=AUTOTUNE_WINO_HW_INPUT_CONFIGS,
        key=["T_v", "C_pad"],
        cache_results=True,
    )(_winograd_hw_f4x3_input_transform_kernel)

    _winograd_hw_f4x3_batched_gemm_kernel = triton.autotune(
        configs=AUTOTUNE_WINO_HW_GEMM_CONFIGS,
        key=["T_out", "K_out", "C_pad"],
        cache_results=True,
    )(_winograd_hw_f4x3_batched_gemm_kernel)

    _winograd_hw_f4x3_output_transform_kernel = triton.autotune(
        configs=AUTOTUNE_WINO_HW_OUTPUT_CONFIGS,
        key=["T_out", "K_out"],
        cache_results=True,
    )(_winograd_hw_f4x3_output_transform_kernel)
