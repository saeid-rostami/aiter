...
def _resolve_route(T, R, S, dilation, layout):
    ...
    by layout (NCDHW takes the cblocked input repack, NDHWC the channels-last
    kernel). Everything else -> the im2col-free general kernel.
    """
    if _is_1x1x1_conv(T, R, S, dilation):
        return Route3D.ONE_X_ONE_X_ONE
    if _is_3x3x3_conv(T, R, S) and dilation == (1, 1, 1):
        # Dilated 3x3x3 has no specialized kernel yet -> general.
        if layout == "ndhwc":
...
def _route_and_run(
    ...
    route = _resolve_route(T, R, S, dilation, layout)
